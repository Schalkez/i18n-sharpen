...
### Fixed
- **Array and Complex Value Serialization**: Fixed bug in `flattenObject` which corrupted nested arrays and non-primitive objects during extraction or pruning. Arrays of objects are now preserved cleanly.

### Added
- **`ignoreFallbackKeys` option**: Added configuration parameter to skip fallback copy-paste validation for specific brand names or key patterns.
- **`stubPlaceholder` option**: Added configuration parameter supporting customizable initial values (`"key"`, `"default"`, or any custom string) when new keys are extracted.
