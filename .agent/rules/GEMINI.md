...
| Rule               | Status           | Reason                                 |
| ------------------ | ---------------- | -------------------------------------- |
| **Force Push**     | ❌ **FORBIDDEN** | Rewrites history, breaks collaborators |
| **Regular Push**   | ✅ Allowed       | Safe, preserves history                |
| **Rebase (local)** | ✅ Allowed       | Before first push only                 |

**Protocol:**

1. **Before first push:** Use `git rebase -i` freely to clean up commits
2. **After push:**
   - ❌ **NEVER** use `git push --force` or `--force-with-lease`
   - ✅ Create new commits instead
   - ✅ Or close PR and create new one
3. **To squash commits:** Use GitHub's "Squash and merge" when merging PR
...
**Branch Naming:** `<type>/<JIRA-ID>-<description>`

- Example: `feature/DIFFAPP-635-project-detail-responsive`
...
## EXTENSION-SPECIFIC RULES (Guide-Maker Chrome Extension)

These rules apply when working on the guide-maker-chrome-extension project:

### Coding Standards
- No `any` Type: Avoid using `any` and explicit type casting (e.g., `as T`) unless strictly necessary or mandated by external library integrations.
- Always Use i18n: Never hardcode user-facing strings in the codebase. Always utilize the i18n system (`useTranslation`, translation files, etc.) for UI text.
- Derived Union Types: Use `typeof CONST_OBJECT[keyof typeof CONST_OBJECT]` (or `keyof typeof CONST_OBJECT`) derived from const objects instead of declaring hardcoded union types (e.g., `type X = "A" | "B"`).

### Architecture
- Single Responsibility / No Mix Concerns: One module = one job. Listener lifecycle must not mix with data application logic. Quota evaluation must be separate from sync logic.
- Constants Consolidation: All batch sizes, limits, TTLs, and concurrency numbers must live in src/core/shared/constants/performance.ts. Never hardcode magic numbers in engine code.

### Sync Engine Safety
- Quota Guard: Always check flowMeta?.isLocalOnly before processing upstream sync jobs. Skip and delete jobs for Local Only flows.
- Firestore Timestamp Safety: Always run normalizeTimestamps() before writing Firestore documents to IndexedDB. Firestore Timestamp objects cause inconsistent comparisons in Dexie.
- Bulk Operations: Prefer bulkPut(), bulkDelete(), primaryKeys() over toArray() for large datasets. Use indexed queries when only IDs are needed.
- Memory: Do not embed full step data in syncQueue jobs. Fetch latest data from db.steps at sync processing time.

### Verification
- Type Check: Run tsc --noEmit after any sync engine or database schema change.
