...
export const DEFAULT_CONFIG: Partial<I18nCopConfig> = {
  excludeDirs: ["node_modules", "dist", ".git", ".next", "build", "coverage", ".agent", ".claude"],
  ...
  ignoreKeys: [],
  pluralSuffixes: ["_zero", "_one", "_two", "_few", "_many", "_other", "_male", "_female"]
}
...
export const I18nCopConfigSchema = z.object({
  ...
  defaultLanguage: z.string().nonempty("defaultLanguage must be a non-empty string"),
  supportedLanguages: z.array(z.string()).nonempty("supportedLanguages must contain at least one language"),
  excludeDirs: z.array(z.string()).optional(),
  fileExtensions: z.array(z.string()).optional(),
  matchFunctions: z.array(z.string()).optional(),
  outputReport: z.string().optional(),
  matchAttributes: z.array(z.string()).optional(),
...
export function loadConfig(cwd: string = process.cwd()): I18nCopConfig {
  ...
  if (fs.existsSync(configPathJson)) {
    try {
      ...
      console.warn(`⚠️ Failed to parse i18n-sharpen.json: ${(error as Error).message}`)
    }
  ...
  const rawConfig = {
    ...
    outputReport: fileConfig.outputReport !== undefined ? fileConfig.outputReport : DEFAULT_CONFIG.outputReport,
  ...
  if (!config.supportedLanguages.includes(config.defaultLanguage)) {
    ...
    config.supportedLanguages = Array.from(new Set([config.defaultLanguage, ...config.supportedLanguages]))
  }
